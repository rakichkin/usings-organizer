...
{
    private RegexBasedUsingSectionsExtractor _extractor = null!;

    [SetUp]
    public void SetUp()
    {
        _extractor = new RegexBasedUsingSectionsExtractor();
    }

    [Test]
    public void Extract_SingleUsingStringSingleUsingDirective_ShouldReturnOne()
    {
        const string code = "using System;\nclass C {}";
        var result = _extractor.Extract(code).ToList();
		using(Assert.EnterMultipleScope())
		{
			Assert.That(result.Single().Usings, Is.EqualTo(new[] { "using System;" }));
			Assert.That(result.Single().StartLinePosition, Is.Zero);
			Assert.That(result.Single().EndLinePosition, Is.Zero);
		}
	}

	[Test]
	public void Extract_SingleUsingStringThreeUsingDirectives_ShouldReturnThree()
	{
		const string code = "using System; using System2; using System3;\nclass C {}";
		var result = _extractor.Extract(code).ToList();
		using(Assert.EnterMultipleScope())
		{
			Assert.That(result.Single().Usings, Is.EqualTo(new[]
			{
				"using System;",
				"using System2;",
				"using System3;"
